...
public class WordCountResourceImpl implements WordCountResource {
    ...
    public Response insertWordCount(String url) {
        ...
        try {
            ...
            // valid for our in-memory DB, entry doesn't exist
        }
        ...
        for (String word : words) {
            ...
            if (count.isPresent()) {
                ...
                if (Status.OK.equals(validWord.getStatusInfo())) {
                    ...
                    // LOG some sort of warning since it's not fatal, though it may mean
                    // our count is potentially wrong. Easy to change to an error should
                    // the service be unavailable and we want to reflect that the count could be wrong
